...
async def main_h(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Back to main
    if d == "BACK":
        await q.edit_message_text("مرحبًا! اختر قسمًا:", reply_markup=InlineKeyboardMarkup(build_main_menu()))
...
async def adm_receive_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.document:
        ...
        name = msg.document.file_name if msg.document.file_name else "document_" + msg.document.file_unique_id
    elif msg.video:
        fid = msg.video.file_id
        name = msg.video.file_name if msg.video.file_name else "video_" + msg.video.file_unique_id
    elif msg.audio:
        fid = msg.audio.file_id
        name = msg.audio.file_name if msg.audio.file_name else "audio_" + msg.audio.file_unique_id
    elif msg.photo:
        fid = msg.photo[-1].file_id
        name = "photo_" + msg.photo[-1].file_unique_id + ".jpg" # Telegram photos are usually JPEG
    else:
